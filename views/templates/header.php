<!DOCTYPE html>
<html lang="fr">

<head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <meta name="robots" content="index, follow">
    <meta name="description" content="Colyseum est un projet POO pour la MANU" />
    <meta name="author" content="Thomas Guillemont" />
    <title>Colyseum</title>
    <link rel="icon" type="image/x-icon" href="/public/assets/img/bird.gif" />
    <!-- Bootstrap CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <!-- CSS -->
    <link href="/public/assets/css/style.css" rel="stylesheet">
</head>

<body class="bg-main">
    <header class="p-1 mb-2 bg-nav d-flex">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12 d-flex align-items-center justify-content-center fixed-top bg-nav">
                    <img class="bird me-3" src="/public/assets/img/bird.gif" alt="bird">
                    <h1 class="text-center fw-bold fs-2">Colyseum</h1>
                    <img class="bird ms-3" src="/public/assets/img/bird.gif" alt="bird">
                </div>
                <div class="col-12 d-flex align-items-center justify-content-center mt-5">
                    <ul class="navbar-nav">
                        <li class="nav-item fw-bold fst-italic fs-6 d-flex flex-wrap justify-content-center text-center">
                            <a class="nav-link link p-1" href="/clients">Clients</a>
                            <div class="p-1">/</div>
                            <a class="nav-link link p-1" href="/spectacles">Spectacles</a>
                            <div class="p-1">/</div>
                            <a class="nav-link link p-1" href="/premiers-clients">Premiers clients</a>
                            <div class="p-1">/</div>
                            <a class="nav-link link p-1" href="/fidélités">Cartes</a>
                            <div class="p-1">/</div>
                            <a class="nav-link link p-1" href="/noms">Noms</a>
                            <div class="p-1">/</div>
                            <a class="nav-link link p-1" href="/titres-spectacles">Informations spectacles</a>
                            <div class="p-1">/</div>
                            <a class="nav-link link p-1" href="/informations-clients">Informations clients</a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </header>